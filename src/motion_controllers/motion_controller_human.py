...
from src.utils.helpers import quat2euler, normalize_radians
...
class MotionControllerHuman:
    ...
    def step(self, human, robot, final_ori, path, is_new_end):
        ...
        if len(path) > 0:
            ...
            action = self.action(
                vel[0] / self.MAX_LIN_VEL,
                vel[1] / self.MAX_ANG_VEL,
                0,
                0,
                0,
                0
    ...
    def angle_difference(self, theta, optimal_heading):
        ...
        if optimal_heading - 0.05 < temp < optimal_heading + 0.05:
            return min_theta
        else:
            return -min_theta
