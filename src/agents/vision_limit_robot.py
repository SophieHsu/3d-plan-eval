...
from src.agents.hl_qmdp_agent import MAX_DELAY_TIME, STUCK_TIME_LIMIT, HlQmdpPlanningAgent
...
from src.planners.steak_human_subtask_qmdp_planner import SteakHumanSubtaskQMDPPlanner
from src.utils.constants import TARGET_ORNS
from src.utils.functions import grid_transition
...
class VisionLimitRobotAgent(HlQmdpPlanningAgent):
    ...
    def action(self):
        ...
        mdp_state_keys = \
            self.mdp_planner.world_to_state_keys(self.env.world_state, self.env.robot_state, self.env.human_state, self.belief, self.env)
        self.belief, self.prev_dist_to_feature = \
            self.mdp_planner.belief_update(self.env, self.belief, self.prev_dist_to_feature, self.hl_human_agent)
        ...
        action_idx, action_object_pair, LOW_LEVEL_ACTION = self.mdp_planner.step(self.env, self.belief, 1)

        # TODO: state string is used for updating robot state, so pickup onion gets discarded, proper environment
        #  update will remove this code state_str = f'{robot_state_str}_{human_holding}_pickup_onion'
        ...
        next_state_idx = np.argmax(
            self.mdp_planner.transition_matrix[action_idx][state_idx])
    ...
    def stuck_handler(self):
        ...
        if elapsed > STUCK_TIME_LIMIT:
            ...
            new_ml_goal = self.adjacent_empty_square(self.env.human_state.ml_state)
    ...
    def robot_action_from_overcooked_api(self):
        ...
        if not self.awaiting_response:
            ...
            # try:
            filename = 'src/logs/' + self.env.kitchen.kitchen_name + self.log_dict['log_id'] + '_log_dict.json'
        ...
        if self.ml_robot_action is None and elapsed > 3:
            self.awaiting_response = False
    ...
    def log_state(self):
        filename = 'src/logs/' + self.env.kitchen.kitchen_name + '_log.txt'
        f = open(filename, "a")
        s = '\n'
    ...
    def get_action_object(self, ml_state):
        ...
        if self.env.kitchen.overcooked_robot_holding is not None:
        ...
        elif station_key == 'K' and robot_holding_name is None and self.env.tracking_env.get_closest_green_onion(
                self.ig_robot.object.get_position()).current_index == 0:
            action_object = ('chop', 'onion')
        elif station_key == 'K' and robot_holding_name == 'steak' and self.env.tracking_env.get_closest_green_onion(
                self.ig_robot.object.get_position()).current_index == 1:
        ...
        elif station_key == 'P' and (len(self.env.tracking_env.obj_in_robot_hand()) == 0 or
                 self.env.tracking_env.obj_in_robot_hand()[0][0] == 'hot_plate'):
    ...
    def ll_step(self):
        ...
        if self.ml_robot_action is None:
            return
