...
from src.environment.lsi_env import AgentState
...
from src.utils.constants import DIRE2POSDIFF
...
class HlQmdpPlanningAgent(Agent):
    ...
    def avoidance_motion_plan(self, joint_ml_state, goal, avoid_path, avoid_goal, radius=None):
        ...
        path = self.mlp.compute_motion_plan(joint_ml_state, (avoid_goal, goal), avoid_path, radius)
        return path
    ...
    def hl_human_step(self):
        ...
        if not self.env.human_state.equal_hl(self.human_sim_state.hl_state) or not \
                self.env.human_state.equal_ml(self.human_sim_state.ml_state, facing=True):
            self.hl_human_action = self.hl_human_agent.action(
                self.env.world_state, self.env.human_state,
                self.env.robot_state)
            # TODO update env.human.hl_state to pull from world. currently env.human_state is not getting updated
            #  when human arrives at goal
    ...
    def ml_human_step(self):
        ...
        if (not self.env.human_state.equal_ml(self.human_sim_state.ml_state, facing=True)
            or self.ml_human_action is None) and self.human_ml_plan:
    ...
    def ml_robot_step(self):
        if self.take_ml_robot_step:
            ...
            if self.ml_robot_action and self.env.human_state.ml_state[:2] == \
                    grid_transition(self.ml_robot_action[1], self.env.robot_state.ml_state)[0:2]:
