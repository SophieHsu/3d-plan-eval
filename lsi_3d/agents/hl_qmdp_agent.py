...
class HlQmdpPlanningAgent(Agent):
    ...
    def hl_human_step(self):
        ...
        ) or not self.env.human_state.equal_ml(self.human_sim_state):
    ...
    def ml_human_step(self):
        ...
        if not self.env.human_state.equal_ml(
                self.human_sim_state) or self.ml_human_action == None:
    ...
    def hl_robot_step(self):
        ...
        if self.take_hl_robot_step:
            ...
            plan = self.avoidance_motion_plan(
                (human_pos, self.env.robot_state.ml_state),
                self.robot_goal,
                self.human_ml_plan,
                self.human_goal,
                radius=0)
