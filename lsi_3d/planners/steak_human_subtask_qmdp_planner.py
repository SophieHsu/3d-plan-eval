...
class  SteakHumanSubtaskQMDPPlanner(SteakMediumLevelMDPPlanner):
    def __init__(self, mdp, mlp:AStarMotionPlanner, vision_limited_human = None):
        ...
        # need env for map action to location 
        ...
        for action, action_idx in self.action_idx_dict.items():
            for curr_state, curr_state_idx in self.state_idx_dict.items():
                ...
                for next_state_idx in next_states:
                    ...
                    human_action_key = self.get_key_from_value(
                        self.action_idx_dict, action_idx)
    ...
    def world_state_to_mdp_state_key(self, state, player, other_player, env, subtask=None, RETURN_NON_SUBTASK=False, RETURN_OBJ=False):
    ...
    def world_to_state_keys(self, world_state, player, other_player, belief, env):
        ...
        for i, b in enumerate(self.subtask_dict):
            mdp_state_key = self.world_state_to_mdp_state_key(world_state, player, other_player, env, self.get_key_from_value(self.subtask_idx_dict, i))
            if self.get_mdp_state_idx(mdp_state_key) is not None:
                mdp_state_keys.append(self.world_state_to_mdp_state_key(world_state, player, other_player, env, self.get_key_from_value(self.subtask_idx_dict, i)))
        ...
        for i, mdp_state_key in enumerate(mdp_state_keys):
            ...
            if mdp_state_idx is not None and belief[used_belief[i]] > 0.01:
                ...
                for j, action_idx in enumerate(agent_action_idx_arr): # action_idx is encoded subtask action
                    ...
                    after_action_world_state, cost, goals_pos = self.mdp_action_state_to_world_state(action_idx, mdp_state_idx, env, with_argmin=True)
    ...
    def belief_update(self, env, belief_vector, prev_dist_to_feature, sim_human_agent, greedy=False, vision_limit=False, prev_max_belief=None):
        """
        ...
        # if vision_limit:
        # self.sim_human_model.update(env.world_state)
        ...
        num_item_in_pot, chop_time, wash_time, robot_obj = self.kb_to_state_info(sim_human_agent.human_sim.knowledge_base)
        ...
        for i, belief in enumerate(belief_vector):
            ...
            feature_pos = env.map_action_to_location((self.subtask_dict[subtask_key[i]][0], self.subtask_dict[subtask_key[i]][1]), human_pos_and_or)# p0_obj=human_obj, player_idx=(1-self.agent_index), counter_drop=True) 
    ...
    def compute_mdp(self, orders, filename=None, ):
        ...
        # with open('value_policy_matrix.pkl', 'wb') as output:
        #     pickle.dump([self.value_matrix, self.policy_matrix], output, pickle.HIGHEST_PROTOCOL)
